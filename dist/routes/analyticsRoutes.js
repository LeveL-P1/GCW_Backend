...
const sessionAccess_1 = require("../middleware/sessionAccess");
const router = (0, express_1.Router)();
async function ensureParticipantAccess(sessionId, userId) {
    if (!userId) {
        return false;
    }
    return (0, sessionAccess_1.isSessionParticipant)(sessionId, userId);
}
router.get("/metrics/:sessionId", async (req, res) => {
    ...
    const latestSnapshot = await prisma_1.prisma.metricsSnapshot.findFirst({
        where: { sessionId },
        orderBy: { timestamp: "desc" }
    });
    if (latestSnapshot) {
        const liveMetrics = (0, telemetryEngine_1.getSessionMetrics)(sessionId);
        const perUser = liveMetrics
            ? Array.from(liveMetrics.userEdits.entries()).map(([userId, count]) => ({
                userId,
                edits: count
            }))
            : [];
...
router.get("/metrics/:sessionId/timeline", async (req, res) => {
    ...
    const events = await prisma_1.prisma.eventLog.findMany({
        where: { sessionId },
...
router.get("/metrics/:sessionId/mode-transitions", async (req, res) => {
    ...
    const snapshots = await prisma_1.prisma.metricsSnapshot.findMany({
        where: { sessionId },
        orderBy: { timestamp: "asc" }
    });
    if (!snapshots.length) {
...
router.post("/mode/:sessionId", async (req, res) => {
    ...
    await (0, governanceEngine_1.hydrateSessionState)(sessionId);
    const role = await (0, governanceEngine_1.getPersistedRole)(sessionId, effectiveUserId);
    if (role !== "FACILITATOR") {
        return res.status(403).json({ message: "Only facilitator can change mode" });
    }
    await (0, governanceEngine_1.setMode)(sessionId, mode);
    (0, connectionManager_1.broadcast)(sessionId, {
        type: "MODE_CHANGED",
        sessionId,
        mode
    });
